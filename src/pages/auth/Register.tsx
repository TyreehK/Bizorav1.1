...
import { ArrowRight, Eye, EyeOff, Mail, User, Lock, Sparkles, ShieldCheck } from "lucide-react";
...
export default function Register() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      const hint = error.status === 500 ? "Controleer dat 'Confirm email' UIT staat in Supabase." : "";
        ...
        <div className="order-2 md:order-1">
          <Card className="card p-6 md:p-8">
            <CardHeader className="p-0">
              <CardTitle className="text-2xl md:text-3xl tracking-tight">Account aanmaken</CardTitle>
              <p className="text-white/70 text-sm mt-2">Maak je Bizora-account aan en ga direct aan de slag.</p>
            </CardHeader>
              ...
              <form className="space-y-5" onSubmit={onSubmit} noValidate>
                <div>
                  ...
                  {!nameValid && fullName.length > 0 && (
                    <p className="text-xs text-red-400 mt-1">Voer minimaal 2 tekens in.</p>
                  )}
                </div>
                ...
                <Button type="submit" disabled={!formValid || loading} className="btn-primary w-full">
                  {loading ? "Bezig..." : <span className="inline-flex items-center">Account aanmaken <ArrowRight className="ml-2 h-4 w-4" /></span>}
                </Button>
                ...
                <div className="flex justify-between text-sm text-white/80">
                  <Link to="/login" className="underline">Ik heb al een account</Link>
        ...
        <div className="order-1 md:order-2">
          <div className="card p-8 md:p-10 relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <h2 className="mt-4 text-3xl md:text-4xl font-extrabold tracking-tight">Binnen 1 minuut gestart.</h2>
              <p className="mt-3 text-white/75">Consistente UI, inline feedback en een soepele eerste ervaring.</p>
              <ul className="mt-6 space-y-3">
                {[
                  { icon: <ShieldCheck className="h-4 w-4" />, text: "Supabase-auth, RLS-proof" },
                  { icon: <Sparkles className="h-4 w-4" />, text: "Glas-effect, zachte schaduwen" },
